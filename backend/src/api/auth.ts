...
import { DB } from "../db";
import { base64urlDecode, convertEncoding, fromBinary, randomBytes, sha256, utcTimestamp } from "../utilty";
...
export async function auth(req: ReqType, res: ResType, data: ApiReq[ApiCode.Auth]): Promise<ApiRes[ApiCode.Auth]> {
  ...
  if (sha256(validator) !== result[0].validator) return res.send({ err: ApiError.AuthFail });
